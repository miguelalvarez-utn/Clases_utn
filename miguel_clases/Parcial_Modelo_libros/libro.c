...
*
*/
int libro_alta(Libro array[], Autor array2[],int size, int size2, int* contadorID)                        //cambiar libro
{
    int retorno=-1;
    int posicion;
    int auxId;
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n idAutor: %d\ntitulo: %s\n",
                       array[i].idUnico,array[i].idAutor,array[i].titulo);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
