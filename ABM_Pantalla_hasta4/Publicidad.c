...
void pub_printContrataciones(Publicidad* pPublicidad,Pantalla * pPantalla, int lenPub, int lenPan)
{
    int i;
    int posPant;
    for(i=0; i<lenPub; i++)
    {
        if(pPublicidad[i].isEmpty==0)
        {
            posPant <= pan_findById(pPantalla,lenPan,pPublicidad)
            if(posPant>=0)
            {
                printf("IDpub:%d --- NombrePan:%s\n --- %s --- %d ---%s\n",
                pPublicidad[i].id, pPantalla[posPant].nombre,
                pPantalla[i].archivo, pPantalla[posPant].dias
                pPantalla[posPant].cuit);
            }
    }
    return 0;
}
