...
int pan_alter(Pantalla* pPantalla, int len,char* msgE,int tries)
{
    ...
    if((pPantalla!=NULL)&&(len>0))
    {
        auxID=pan_getID(pPantalla,len,msgE,tries);///pirde al usuario que ingrese el id de la pantalla
        if(auxID>=0)
        {
            posOfID=pan_findPanById(pPantalla,len,auxID);///busca en el array de pantallas que tenga ese id
            if(posOfID!=-1)
            {
                while(opcion!=5)
                {
